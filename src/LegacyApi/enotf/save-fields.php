...
if (isset($_POST['enr']) && isset($_POST['field'])) {
    ...
    if ($field === 'freigeber') {
        ...
        try {
            ...
            if ($protokoll) {
                // Domain-Event feuern — Listener dispatchen den Discord-Webhook-Job
                app(\App\Events\EventDispatcher::class)->fire(
                    new \App\Events\EnotfProtocolReleased($protokoll)
                );
            }
        } catch (\Throwable $e) {
            \App\Logging\Logger::error('EnotfProtocolReleased: Event-Fire Fehler', [
                'error' => $e->getMessage(),
            ]);
        }
