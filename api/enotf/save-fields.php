...
if (isset($_POST['enr']) && isset($_POST['field'])) {
    ...
    // Block writes on released protocols (except the freigeber action itself)
    if ($field !== 'freigeber') {
        $chkStmt = $pdo->prepare("SELECT freigegeben FROM intra_edivi WHERE enr = :enr");
        $chkStmt->execute(['enr' => $enr]);
        $chkRow = $chkStmt->fetch(PDO::FETCH_ASSOC);
        if ($chkRow && $chkRow['freigegeben'] == 1) {
