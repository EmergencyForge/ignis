...
use App\Policies\FahrtPolicy;
use PHPUnit\Framework\Attributes\Test;
use Tests\TestCase;

/**
 * Coverage für die wichtigste Multi-Auth-Policy: FahrtPolicy darf
 ...
 */
class FahrtPolicyTest extends TestCase
{
    ...
    {
        $this->loginAdmin([]);
        $this->assertFalse(FahrtPolicy::viewList());

        $this->loginAdmin(['fahrtenbuch.view']);
        $this->assertTrue(FahrtPolicy::viewList());
    ...
    {
        $this->loginAdmin();
        $this->assertTrue(FahrtPolicy::create());
    }
    ...
    {
        $this->assertFalse(FahrtPolicy::create());
    }
    ...
    {
        $this->loginAdmin(['fahrtenbuch.manage']);
        $entry = $this->makeFahrt([
            'created_by'  => 999,
            'source'      => Fahrt::SOURCE_ENOTF,
            'fahrer_name' => 'Other',
        ]);
        $this->assertTrue(FahrtPolicy::update($entry));
    }
    ...
    public function update_enotf_session_cannot_update_others_enotf_entry(): void
    {
        ...
        $this->assertFalse(FahrtPolicy::update($entry));
    }
    ...
    {
        $this->loginEnotf();
        $this->assertTrue(FahrtPolicy::update(null));

        $_SESSION = [];
        $this->assertFalse(FahrtPolicy::update(null));
    }
    ...
    {
        $this->loginAdmin(['fahrtenbuch.manage']);
        $this->assertTrue(FahrtPolicy::delete());

        $this->loginAdmin(['fahrtenbuch.view']);
        $this->assertFalse(FahrtPolicy::delete());
    }
    ...
    public function delete_rejects_enotf_session_even_for_own_entry(): void
    {
        ...
        $this->assertFalse(FahrtPolicy::delete($entry));
    }
