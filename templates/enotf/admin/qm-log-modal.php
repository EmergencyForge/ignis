...
<div class="w-full">
    ...
    if (count($log_result) == 0) {
        echo "<div class='edivi__box'><p class='text-center text-gray-400'>Keine Einträge vorhanden.</p></div>";
    } else {
        foreach ($log_result as $log_row) {
            $log_row['timestamp'] = date("d.m.Y H:i", strtotime($log_row['timestamp']));
            if ($log_row['log_aktion'] == 0) {
    ?>
                <div class='edivi__box edivi__log-comment mb-3 flex items-center gap-3'>
                    <div class="flex h-8 w-8 items-center justify-center"><i class="fa-solid fa-circle-info"></i></div>
                    <div class='flex-1'>
            ...
            ?>
                <div class='edivi__box edivi__log-comment mb-3 flex items-center gap-3'>
                    <div class="flex h-8 w-8 items-center justify-center"><i class="fa-solid fa-gear"></i></div>
                    <div class='flex-1'>
                        <small style="opacity:.6" class='mb-0'><b><?= htmlspecialchars($log_row['bearbeiter']) ?></b> | <?= $log_row['timestamp'] ?></small>
